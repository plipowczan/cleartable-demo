...
- **M1 zawsze fundament**, ostatni milestone zawsze **deploy + analytics**
...
| M5 | deploy + analytics | publiczny URL, UTM, `data-analytics-event` na core flow, smoke test | 40–60 tur / pół dnia |

Zasady kolejności (absolutne): M1 fundament pierwszy · dane przed UI · deploy + analytics ostatni · sekwencyjnie, bez równoległości · NIE rób baseline OpenSpec (overhead w 7 dni). Jeśli Core Flow jest mały, możesz połączyć M3+M4 → wtedy 4 milestone'y zamiast 5 (to OK).
...
- [ ] 3–5 milestone'ów, M1 = fundament, ostatni = deploy + analytics
...
- **M4 edge cases** — race przy podwójnej rezerwacji, UTM, error "slot właśnie zajęty", status "available now"
- **M5 deploy + handoff** — publiczny URL, `data-analytics-event`, lista eventów dla W4
